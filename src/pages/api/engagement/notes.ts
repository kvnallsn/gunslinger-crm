...
export default async function handler(
    ...
    const notes = await Engagement.fetchNotes(db, session.user.id, id);
    db.release();

    return res.status(200).json(NewAppResponse(200, notes));
}